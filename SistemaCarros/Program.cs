    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (MenudeOpcoes() == 1)
            {
                Console.Clear();
                TeladeInicio();
                Console.WriteLine("Carro a ser alugado:");
                ...
                if (PesquisaCarrosDisponiveis(nomedocarro))
                {
                    ...
                    {
                        Console.WriteLine($"Carro: {bancodeCarros[i, 0]}\r Ano: {bancodeCarros[i, 1]}\r Disponibilidade: {bancodeCarros[i, 2]}");
                    }
                }
            }
        ...
        public static bool PesquisaCarrosDisponiveis(string nomedocarro)
        {
            for (int i = 0; i < bancodeCarros.GetLength(0); i++)
            {
                if (nomedocarro == bancodeCarros[i, 0])
                {
                    ...
                    return bancodeCarros[i, 2] == "Disponível para aluguel";
                }
            }
        ...
        public static void AlugarCarro(string nomedocarro)
        {
            ...
            {
                if (nomedocarro == bancodeCarros[i, 0])
                    bancodeCarros[i, 2] = "não disponível";
