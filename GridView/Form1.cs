    ...
    public partial class Form1 : Form
    {
        ...
        private void DataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > - -1)
            {
                ...
                var colValue = dataGridView1.Rows[e.RowIndex].Cells[e.ColumnIndex];
                    ...
                    case 1:
                        {
                                ...
                                , MessageBoxIcon.Question) == DialogResult.Yes)
                            {
                                ...
                                x.Id == (int)columnId.Value).Value = (int)colValue.Value;
                            }
                            #endregion
                        }
                        break;
                    case 2:
                        {
                            ...
                            {
                                var dataInformada = DateTime.Parse(colValue.Value.ToString());

                                if (dataInformada <= DateTime.Now)
                                    listContracs.FirstOrDefault(x =>
                                    x.Id == (int)columnId.Value).DatInc = DateTime.Parse(colValue.Value.ToString());
                                else
